    ...
    return (
      <AurumLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 space-y-4">
          ...
          <h2 className="text-xl font-bold text-foreground">Profiel niet gevonden</h2>
          <p className="text-muted-foreground mb-4">Er is een probleem met het laden van je profiel.</p>
    ...
    <AurumLayout>
      <div className="px-6 py-8 md:py-12 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start md:gap-8 mb-12">
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-gradient-to-br from-secondary to-muted border-2 border-amber-500/20 flex items-center justify-center mb-4 md:mb-0 shadow-xl overflow-hidden flex-shrink-0">
            {user.avatarUrl ? (
              <img src={user.avatarUrl} alt={user.name} className="w-full h-full object-cover" />
            ) : (
              <span className="text-3xl md:text-4xl font-display font-bold text-amber-500">
                {user.name.charAt(0)}
              </span>
            )}
          </div>
        ...
        <div className="max-w-2xl mx-auto md:mx-0 space-y-6">
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Accountgegevens</h3>
          <div className="luxury-card p-6 rounded-2xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0 pb-4 border-b border-border">
              <span className="text-muted-foreground text-sm">E-mailadres</span>
              <span className="text-foreground text-sm font-medium">{user.email}</span>
            </div>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-0">
              <span className="text-muted-foreground text-sm">Registratiedatum</span>
              <span className="text-foreground text-sm font-medium">{new Date(user.createdAt || Date.now()).toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
            </div>
          </div>
          <div className="pt-4">
            <Button
...
function StatCard({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
  return (
    <div className="luxury-card p-5 rounded-2xl flex flex-col items-center justify-center text-center gap-3 hover:bg-secondary/80 transition-colors group">
