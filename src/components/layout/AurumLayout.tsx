      ...
      {showNav && (
        <aside className="hidden md:flex w-72 flex-col fixed inset-y-0 left-0 z-50 liquid-nav border-r">
          ...
          <div className="p-8 border-t border-white/5">
            <div className="p-4 rounded-xl bg-gradient-to-br from-neutral-900 to-black border border-white/5 relative overflow-hidden group">
              <div className="absolute inset-0 bg-amber-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <p className="text-xs text-neutral-400 relative z-10 text-center font-light tracking-wide">
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive && !isPrimary ? "text-amber-400" : "text-neutral-500",
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <span className={cn("transition-colors relative z-10", isActive ? "text-amber-400" : "text-neutral-500 group-hover:text-neutral-300")}>
            {icon}
          </span>
