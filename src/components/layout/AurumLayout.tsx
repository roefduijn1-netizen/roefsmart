...
export function AurumLayout({ children, showNav = true }: AurumLayoutProps) {
  ...
  useNotifications();
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-amber-500/30 flex flex-col lg:flex-row overflow-hidden transition-colors duration-300">
      ...
      {/* Desktop/Tablet Sidebar Navigation (Visible on Large Screens) */}
      {showNav && (
        <aside className="hidden lg:flex w-72 flex-col fixed inset-y-0 left-0 z-50 liquid-nav border-r">
      ...
      <main className={cn(
        ...
        // Mobile/Tablet padding for bottom nav
        showNav ? "pb-28 lg:pb-0" : "pb-0",
      ...
      {/* Mobile/Tablet Bottom Navigation (Visible on Small/Medium Screens) */}
      {showNav && (
        <nav className="lg:hidden fixed bottom-6 left-4 right-4 z-50 flex justify-center pointer-events-none">
      ...
      <Toaster
        ...
        position="top-center"
        toastOptions={{
          className: "bg-[#0a0a0a]/90 border border-white/10 text-foreground shadow-2xl backdrop-blur-xl",
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive && !isPrimary ? "text-amber-400" : "text-muted-foreground",
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <span className={cn("transition-colors relative z-10", isActive ? "text-amber-400" : "text-muted-foreground group-hover:text-foreground")}>
            {icon}
          </span>
